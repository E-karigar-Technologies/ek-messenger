 ...
 async sendRecordingFromPreview() {
  ...
  async ionViewWillEnter() {
    try {
      ...
      await this.checkAnnouncementGroupAccess();
  ...
  private async checkAnnouncementGroupAccess(): Promise<void> {
    try {
      if (!this.roomId) return;

      const freshSenderId = this.authService.authData?.userId || this.senderId;
      if (!freshSenderId) return;

      const db = getDatabase();

      const groupSnap = await get(ref(db, `groups/${this.roomId}`));
      if (!groupSnap.exists()) return;

      const groupData = groupSnap.val();
      const groupTitle = (groupData.name || groupData.title || '').trim();
      this.isAnnouncementGroup = groupTitle === 'Announcements';

      if (!this.isAnnouncementGroup) {
        this.isCommunityOwner = false;
        return;
      }

      const communityId = groupData.communityId;
      if (!communityId) {
        this.isCommunityOwner = false;
        return;
      }

      const communitySnap = await get(ref(db, `communities/${communityId}`));
      if (!communitySnap.exists()) {
        this.isCommunityOwner = false;
        return;
      }

      this.isCommunityOwner =
        String(communitySnap.val().createdBy) === String(freshSenderId);

      console.log('📢 Announcement check:', {
        isAnnouncement: this.isAnnouncementGroup,
        isOwner: this.isCommunityOwner,
        freshSenderId,
        createdBy: communitySnap.val().createdBy,
      });
      ...
      console.error('checkAnnouncementGroupAccess error:', error);
      this.isAnnouncementGroup = false;
      this.isCommunityOwner = false;
    }
  }
      ...
      const alert = await this.alertCtrl.create({
        ...
        buttons: [
          { text: 'Cancel', role: 'cancel' },
          {
            ...
            handler: async () => {
              try {
                // ✅ SECURE UPDATE: Use FirebaseChatService.exitGroup to handle member migration and admin reassignment via socket
                await this.chatService.exitGroup(groupId, [userId as string]);
  ...
  async checkIfBlocked() {
    ...
    const unsubA = onValue(this.iBlockedRef, (snap) => {
      ...
      const val = snap.val();
      const isBlocked = exists && val?.status === 'active';
      this.zone.run(() => {
...
async unblockFromChat() {
    try {
      const db = getDatabase();
      const blockRef = ref(db, `usersBlocks/${this.senderId}/${this.receiverId}`);

      // ✅ Step 1: Get current history
      const snapshot = await get(blockRef);
      if (!snapshot.exists()) return;

      // ✅ SECURE UPDATE: Use FirebaseChatService.unblockUserByUserId (proxied via socket)
  ...
  async deleteChat() {
    try {
      // ✅ SECURE UPDATE: Use FirebaseChatService.clearChatForUser (proxied via socket)
  ...
  //     const attemptLoadAndScroll = async () => {
  ...
  private normalizeTs(ts: string | number | Date | undefined | null): number {
    ...
    if (typeof ts === 'string') {
      ...
      if (isNaN(parsed.getTime())) {
        console.warn('Invalid date string:', ts);
        return 0;
      }
  ...
  private applyDeletionFilters(dm: Message): boolean {
    try {
      // Do NOT hide globally deleted messages → show placeholder in template
      if (
        (dm as any)?.deletedFor?.everyone === true ||
        (dm as any).deletedForEveryone === true
      ) {
        return false;
      }

      // Hide if deleted for current user (array-based)
      if (Array.isArray((dm as any)?.deletedFor?.users)) {
        if ((dm as any).deletedFor.users.includes(String(this.senderId)))
          return true;
      }

      // Hide if legacy map-based per-user deletion is set
      if (
        dm.deletedFor &&
        this.senderId &&
        (dm.deletedFor as any)[String(this.senderId)]
      )
        return true;
  ...
  async handlePopoverAction(action: string) {
    switch (action) {
      ...
      case 'unpin':
        if (this.chatType === 'group' && !this.canPinMessage) {
          const unpinAlert = await this.alertCtrl.create({
            header: 'Permission Denied',
            message: 'Only admins can unpin messages in this group.',
            buttons: [{ text: 'OK', role: 'cancel' }],
          });
          await unpinAlert.present();
          this.selectedMessages = [];
          this.lastPressedMessage = null;
          return;
        }
  ...
  isMessageHiddenForUser(msg: any): boolean {
    ...
    if (msg.deletedFor && Array.isArray(msg.deletedFor.users)) {
      ...
      if (users.includes(String(this.senderId))) {
        return true;
      }
    }
  ...
  async addReaction(msg: IMessage, emoji: string) {
    ...
    const userId = this.senderId;
    const current = msg.reactions?.find((r: { userId: string; }) => r.userId == userId) || null;
  ...
  getReactionsCount(msg: IMessage) {
    return msg.reactions.filter((r: { emoji: any; }) => !!r.emoji).length || 0;
  }
  ...
  getToastLeaveAnimation(baseEl: HTMLElement) {
    ...
    openDefaultTimer() {
      this.router.navigate(['/default-message-timer']);
        ...
        /**
       * ✅ Check if message text contains a app invite link
       */
      getInviteLinkFromText(text: string): string | null {
        if (!text) return null;
        const match = text.match(/https?:\/\/ekmessenger\.com\/join\/\S+/);
       ...
       */
      parseMessageWithLink(text: string): { before: string; link: string | null; after: string } {
        if (!text) return { before: '', link: null, after: '' };
        const match = text.match(/([\s\S]*?)(https?:\/\/ekmessenger\.com\/join\/\S+)([\s\S]*)/);
      ...
      async onInviteLinkTap(link: string, event?: Event): Promise<void> {
        ...
        // Extract groupId from the link: https://ekmessenger.com/join/group_XXXXX
