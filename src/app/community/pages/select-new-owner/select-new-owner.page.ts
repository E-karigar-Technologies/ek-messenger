  ...
  async adminsWithDeviceNames(admins: any[]): Promise<any[]> {
    try {
      ...
      return admins.map((admin) => {
        const byId = nameByUserId.get(String(admin.user_id || admin.userId || ''));
        if (byId) {
          return { ...admin, username: byId };
        }
        ...
        return {
          ...
          username: deviceContact
            ...
            : admin.phoneNumber || admin.phone || admin.phone_number || admin.username,
        };
      });
    } catch (error) {
