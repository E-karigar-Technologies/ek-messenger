...
export class ProfileSetupPage implements OnInit, OnDestroy {
 ...
 async openEmojiKeyboard() {
  ...
  async showToast(
    message: string,
    color: 'danger' | 'primary' | 'dark' = 'dark'
  ) {
    const toast = await this.toastController.create({
      message,
      duration: 3000,
  ...
  async onSubmit() {
    ...
    try {
      ...
      await loading.dismiss();
      await this.showToast('Profile setup completed successfully!', 'primary');
    } catch (error) {
      await loading.dismiss();
  ...
  try {
    ...
    if (!snapshot.exists()) {
      ...
      // ✅ SECURE UPDATE: Use socket proxy to update profile name and activity
      await this.firebaseChatService.applySecuredBatchUpdates({
        [`users/${this.userID}/name`]: this.name,
        [`users/${this.userID}/lastActive`]: new Date().toISOString(),
      });
    }
