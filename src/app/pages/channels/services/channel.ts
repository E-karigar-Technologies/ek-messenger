...
export class ChannelService {
   ...
   * Accepts FormData with: channel_name, description, category_id, region_id, channel_dp (file)
   * Response: { status: boolean, message?: string }
   */
  updateChannel(channelId: number | string, form: FormData): Observable<ApiResponse & { channel?: Channel }> {
    return this.http
      .patch<ApiResponse & { channel?: any }>(this.url(`${channelId}`), form)
      .pipe(
        map(res => {
          if (res && res.channel) {
            return { ...res, channel: this.normalizeChannel(res.channel) };
 ...
 * PATCH /channels/{channelId}/members/{userId}/role
 * Body: { role_id: number, requester_id: number }
 * 
 * Role ID values (based on API documentation):
 * - 1 = Owner
 * - 2 = Admin (or follower based on context)
 * - 3 = Member/Follower
 * 
 * Note: You need to confirm with backend team what role_id represents admin
 */
updateMemberRole(
  channelId: number | string,
  userId: number | string,
...
): Observable<ApiResponse & { member?: Member }> {
  const body = {
    role_id: roleId,
    requester_id: String(requesterId)
  };

  const url = `${this.baseUrl}/${this.resource}/${channelId}/members/${userId}/role`;
  return this.http
    .patch<ApiResponse & { member?: Member }>(url, body, { headers: this.jsonHeaders })
    .pipe(catchError(err => this.handleError(err)));
}

/**
 * Make a user an admin of a channel
 * Convenience method that calls updateMemberRole with admin role_id
 * 
 * @param channelId - The channel ID
 * @param userId - The user ID to make admin
 * @param requesterId - The user ID making the request (usually channel owner)
 * @param adminRoleId - The role_id for admin (default: 2, confirm with backend)
 */
makeUserAdmin(
  channelId: number | string,
  userId: number | string,
  requesterId: number | string,
  adminRoleId: number = 2
): Observable<ApiResponse & { member?: Member }> {
  return this.updateMemberRole(channelId, userId, adminRoleId, requesterId);
}

/**
 * Invite follower to a channel
 * POST /channels/{channelId}/followers/invite
 * Body: { inviter_id: number | string, follower_ids: (number | string)[] }
 */
inviteFollower(
    channelId: number | string,
    inviterId: number | string,
    followerIds: (number | string)[]
  ): Observable<ApiResponse> {
    const body = {
      inviter_id: String(inviterId),
      follower_ids: followerIds.map(id => Number(id))
    };

    const url = `${this.baseUrl}/${this.resource}/${channelId}/followers/invite`;
    return this.http
      .post<ApiResponse>(url, body, { headers: this.jsonHeaders })
      .pipe(catchError(err => this.handleError(err)));
  }

  /**
   * Invite admin to a channel
   * POST /channels/{channelId}/admins/invite
   * Body: { inviter_id: number | string, admin_ids: (number | string)[] }
   */
  inviteAdmin(
    channelId: number | string,
    inviterId: number | string,
    ...
    const body = {
      inviter_id: String(inviterId),
      admin_ids: adminIds.map(id => Number(id))
    };

    const url = `${this.baseUrl}/${this.resource}/${channelId}/admins/invite`;
    return this.http
      .post<ApiResponse>(url, body, { headers: this.jsonHeaders })
      .pipe(catchError(err => this.handleError(err)));
  }
