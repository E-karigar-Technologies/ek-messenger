...
async saveChannel(channel: Channel, immediate: boolean = false): Promise<void> {
  ...
  try {
    ...
    console.log('✅ Deleted cached channel:', channelId);
  } catch (err: any) {
    if (err.status !== 404) {
      console.error('❌ Failed to delete cached channel:', err);
    }
  }
}
 ...
 private saveTimers: Map<string, any> = new Map(); // Debounce timers
  ...
  /* =========================
     FOLLOWERS - CACHED FOR OFFLINE
     ========================= */

  async saveFollowers(channelId: number | string, followers: any[]): Promise<void> {
    try {
      const docId = `followers_${channelId}`;
      try {
  ...
  async getFollowers(channelId: number | string): Promise<any[]> {
    try {
      const docId = `followers_${channelId}`;
      const doc: any = await this.db.get(docId);
