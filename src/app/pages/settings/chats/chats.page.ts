...
export class ChatsPage implements OnInit {
  ...
  loadSettings() {
    try {
      ...
      if (raw) {
        ...
        if (typeof s.translationEnabled === 'boolean') this.translationEnabled = s.translationEnabled;
      }
      // Sync toggle with consent granted from chatting screen
      const consent = localStorage.getItem(this.TRANSLATION_CONSENT_KEY);
      if (consent === 'granted' && !this.translationEnabled) {
        this.translationEnabled = true;
        this.saveSettings();
      }
    } catch (e) {
  ...
  saveSettings() {
    ...
    this.globalSettingsSync.saveSection('chats', s);
  }
