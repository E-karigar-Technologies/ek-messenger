...
export class ApiService {
  ...
  sendEmailVerification(email: string): Observable<any> {
    return this.http.post<any>(`${this.baseUrl}/api/auth/email/send-verification`, { email });
  }

  /** Verify email OTP */
  verifyEmailOtp(email: string, otp: string): Observable<any> {
    return this.http.post<any>(`${this.baseUrl}/api/auth/email/verify`, { email, otp });
  }

  /** Resend email verification OTP */
  resendEmailVerification(email: string): Observable<any> {
    return this.http.post<any>(`${this.baseUrl}/api/auth/email/resend`, { email });
  }

  /** Check email verification status */
  checkEmailVerificationStatus(): Observable<any> {
    return this.http.get<any>(`${this.baseUrl}/api/auth/email/status`);
  }
  ...
  getFirebaseToken(
    bearerToken: string
  ): Observable<{ status: boolean; firebase_token: string }> {
    return this.http.post<{ status: boolean; firebase_token: string }>(
      `${this.baseUrl}/api/auth/firebase-token`,
      {},
      {
        headers: {
  ...
  updateUserLanguage(user_id: number, language: string): Observable<any> {
    return this.http.post(`${this.baseUrl}/api/users/update-language`, {
      user_id,
      language,
    });
  }
   ...
   */
  submitReport(payload: SubmitReportPayload): Observable<SubmitReportResponse> {
    return this.post<SubmitReportResponse>('/api/reports/submit', payload);
  }

  /**
   * Get all available report categories
   */
  getReportCategories(): Observable<GetReportCategoriesResponse> {
    return this.get<GetReportCategoriesResponse>('/api/reports/categories');
  }
  checkPlatformUserWhileAddingContact(phone_hash: string): Observable<any> {
    return this.http.post(`${this.baseUrl}/api/user-contacts/check-by-hash`, { phone_hash });
  }
}
