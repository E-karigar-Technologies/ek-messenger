...
export interface IGroupPermissions {
  editGroupSettings: boolean;  // default: true
  sendMessages: boolean;       // default: true
  addMembers: boolean;         // default: true
  inviteViaLink: boolean;      // default: false
 ...
 export interface IMessage {
  ...
  sender_phone?: string;
  receiver_id?: string;
  type: 'text' | 'image' | 'audio' | 'video' | 'pdf' | 'other' | 'channel_invite';
  ...
  reactions?: { userId: string; emoji: string | null }[] | any;
  replyToMsgId?: string;
  isEdit?: boolean;
  isPinned?: boolean;
  isForwarded?: boolean;
...
export interface IConversation {
  ...
  isTyping?: boolean;
  theyBlocked?: boolean;
  disappearingDuration?: '2' | '24' | '7' | '90' | 'off' | null;
}
...
export interface ICommunity {
  ...
  settings?: {
    announcementPosting?: 'all' | 'adminsOnly';
    whoCanAddMembers?: 'everyone' | 'only_admins';
    whoCanAddGroups?: 'everyone' | 'only_admins';
  };
}
...
const TABLE_SCHEMAS = {
  ...
  messages: `
    CREATE TABLE IF NOT EXISTS messages (
      ...
      channel_invite TEXT,                     -- JSON string for channel invite
...
export class SqliteService {
  ...
  async saveMessage(message: IMessage & { ownerId: string }) {
    ...
    return this.withOpState('saveMessage', async () => {
      ...
      const sql = `
      INSERT INTO messages 
      (msgId, roomId, ownerId, sender, sender_name, receiver_id, type, text, translations, channel_invite, mediaId, isMe, status, timestamp, receipts, deletedFor, replyToMsgId, reactions, isEdit)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
      ...
      const params = [
        ...
        message.channel_invite ? JSON.stringify(message.channel_invite) : null,
        message.mediaId ?? null,
        message.isMe ? 1 : 0,
        message.status ?? null,
        String(message.timestamp),
    ...
    async () => {
      ...
      return res.values.reverse().map((msg) => {
        ...
        return {
          ...
          channel_invite: msg.channel_invite ? JSON.parse(msg.channel_invite) : null,
          receipts: JSON.parse(msg.receipts || '{}'),
          reactions: JSON.parse(msg.reactions || '[]'),
          deletedFor: JSON.parse(msg.deletedFor || '{}'),
          isMe: !!msg.isMe,
